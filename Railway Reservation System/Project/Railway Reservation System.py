...
#--------------MySQL connection---------------#
connection_key=simpledialog.askstring("Database Key","Enter the Database Connection Key")
...
try:
    ...
    if mycon.is_connected():# Checking Connection
        ...
        messagebox.showinfo("Connection Successfull","Successfully connected to Database")   
        ...
        main_win.iconphoto(False,tkinter.PhotoImage(file='Project Setup\\Images\\locomotive.png'))
        ...
        def loginMenu():
            ...
            menu_win.iconphoto(True,tkinter.PhotoImage(file='Project Setup\\Images\\locomotive.png'))
            menu_win.grab_set()
            menu_win.title("Menu")
        ...
        def book_train():
            ...
            def show_trains():
                ...
                def saveFile():
                    ...
                    downl_loc=os.environ["USERPROFILE"]+"\\Downloads"
                    if os.path.exists(downl_loc+"\\tickets")==False:# if path not exists
                        os.mkdir(downl_loc+"\\tickets")# make tickets directory
                        time.sleep(2)
                                
                    file = open(downl_loc+"\\tickets\\"+file_name+".txt","w")
                    with file:
                        pnr = str(data[-1][0])
                        username = data[-1][1]
                        doj = data[-1][2]
